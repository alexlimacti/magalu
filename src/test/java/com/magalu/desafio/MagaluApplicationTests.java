...
public class MagaluApplicationTests {
	...
	@Test
	public void analisadorPlayerKilledWorld() {
		...
		gameLog.addLog("20:34 ClientUserinfoChanged: 2 n\\Isgalamido\\t\\0\\model\\uriel/zael\\hmodel\\uriel/zael\\g_redteam\\\\g_blueteam\\\\c1\\5\\c2\\5\\hc\\100\\w\\0\\l\\0\\tt\\0\\tl\\0");
		gameLog.addLog("20:54 Kill: 1022 2 22: <world> killed Isgalamido by MOD_TRIGGER_HURT");
		...
		Assert.assertEquals("Isgalamido", players.get(0).getNickName());

		final List<PlayerDeath> playerDeaths = games.get(0).getPlayerDeaths();
		Assert.assertEquals(1, playerDeaths.size());
		Assert.assertEquals("Isgalamido", playerDeaths.get(0).getPlayer().getNickName());
